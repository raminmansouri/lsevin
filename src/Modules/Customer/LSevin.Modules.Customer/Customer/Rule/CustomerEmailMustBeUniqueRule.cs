using BuildingBlocks.Core.Domain.Primitives;
using BuildingBlocks.Core.Domain.ValueObjects;
using BuildingBlocks.Core.Resources;
using LSevin.Modules.Customer.Customer.Services;
using LSevin.Modules.Customer.Customer.ValueObjects;

namespace LSevin.Modules.Customer.Customer.Rule;

internal sealed class CustomerEmailMustBeUniqueRule(
    ICustomerUniquenessCheckerService customerUniquenessCheckerService,
    Email email,
    CustomerId? id = null
) : IBusinessRule
{
    public bool IsBroken() => !customerUniquenessCheckerService.IsUnique(email, id);

    public string Message => SharedResource.Email_Uniqueness_Error_Message;
}
