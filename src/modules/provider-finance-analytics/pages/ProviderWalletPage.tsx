...
export async function ProviderWalletPage({ params, searchParams }: ModulePageProps) {
  ...
  const [overview, walletAccounts, walletTransactions, payoutAccounts, withdrawals, settlements] = await Promise.all([
    ...
    listProviderWalletTransactions(providerId, 200),
    listPayoutAccounts(providerId),
    listWithdrawalRequests(providerId, 100),
    listSettlementBatches(providerId, 25),
  ]);
