...
namespace BuildingBlocks.Core.Observability.Extensions;
...
internal static class ActivityExtensions
{
    ...
    public static Dictionary<string, object?> GetParentTags(this Activity activity)
    {
        ...
        var parentTags = new Dictionary<string, object?>(StringComparer.Ordinal);
        ...
        var activityInfo = new ActivityInfo
        {
            ...
            Status =
                activity
                    .Tags.FirstOrDefault(tag =>
                        string.Equals(tag.Key, TelemetryTags.Tracing.Otel.StatusCode, StringComparison.Ordinal)
                    )
                    .Value
                ?? "Unknown",
            StatusDescription = activity
                .Tags.FirstOrDefault(tag =>
                    string.Equals(tag.Key, TelemetryTags.Tracing.Otel.StatusDescription, StringComparison.Ordinal)
                )
                .Value,
            ...
            Events = activity
                .Events.Select(e => new ActivityEventInfo
                {
                    ...
                    Attributes = e.Tags.ToDictionary(tag => tag.Key, tag => tag.Value, StringComparer.Ordinal),
                })
                .ToList(),
    ...
    /// <summary>
    /// Sets the exception tags for the activity.
    /// </summary>
    ...
    public static Activity SetExceptionTags(this Activity activity, Exception? ex)
    {
        ...
        activity.AddTag(TelemetryTags.Tracing.Exceptions.Message, ex.Message);
        activity.AddTag(TelemetryTags.Tracing.Exceptions.Stacktrace, ex.ToString());
        activity.AddTag(TelemetryTags.Tracing.Exceptions.Type, ex.GetType().FullName);
