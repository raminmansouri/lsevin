...
namespace BuildingBlocks.Core.Logging.Enrichers;

/// <summary>
/// Represents the log enricher.
/// </summary>
public static class LogEnricher
{
    ...
    public static void EnrichFromRequest(IDiagnosticContext diagnosticContext, HttpContext httpContext)
    {
        ...
        diagnosticContext.Set(
            "UserId",
            httpContext
                .User.FindFirst(x => string.Equals(x.Type, ClaimTypes.NameIdentifier, StringComparison.Ordinal))
                ?.Value
            ?? string.Empty
        );
    ...
    public static LogEventLevel GetLogLevel(HttpContext ctx, double _, Exception? ex) =>
        ...
        : IsHealthCheckEndpoint(ctx) ? LogEventLevel.Debug
        : LogEventLevel.Information;

    /// <summary>
    /// Determines whether the specified context is a health check endpoint.
    /// </summary>
    /// <param name="ctx">The context.</param>
    /// <returns>True if the specified context is a health check endpoint; otherwise, false.</returns>
