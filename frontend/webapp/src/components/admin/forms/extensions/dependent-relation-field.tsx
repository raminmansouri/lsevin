...
export function DependentRelationField({ control, name, label, locale, config }: Props) {
  const controller = useController({ control, name });
  const parentNames = useMemo(() => config.parentFilters?.map((item) => item.parentField) ?? [], [config.parentFilters]);
  ...
  const missingRequiredParent = parentEntries.some((item) => item.required !== false && (item.value === undefined || item.value === null || item.value === ""));
  ...
  }, [config.resetOnParentChange, controller.field, parentSignature]);
  ...
  async function loadOptions({ search, page, pageSize }: { search: string; page: number; pageSize: number }) {
    ...
    url.searchParams.set("parents", JSON.stringify(parentEntries.map((item) => ({ field: item.targetColumn, value: item.value }))));
  ...
  return (
    <div className="space-y-2">
      <AsyncSearchableSingleSelect
        ...
        placeholder={missingRequiredParent ? config.placeholderWhenMissingParents ?? "Select parent first" : config.placeholder ?? `Select ${label.toLowerCase()}`}
