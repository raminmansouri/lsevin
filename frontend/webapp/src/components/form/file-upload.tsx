...
>(
  (
    ...
    const t = useTranslations("Common.FileUpload.errors");
  ...
  return (
    <div
      ...
      className={cn(
        ...
        "relative flex h-6 cursor-pointer items-center justify-between p-1",
        className,
        isSelected ? "bg-muted" : ""
      )}
      {...props}
    >
      <div className="flex h-full w-full items-center gap-1.5 pr-6 leading-none font-medium tracking-tight">
        {children}
      </div>
      <button
        type="button"
  ...
  return (
    ...
    >
      ...
      <Input
        ...
        className={`${isLOF ? "cursor-not-allowed" : ""} hidden`}
      />
    </div>
  );
});
