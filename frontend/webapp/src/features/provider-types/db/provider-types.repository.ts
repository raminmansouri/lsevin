...
export async function getProviderType(id: string): Promise<ProviderType | null> {
  ...
  const attributeRows = await sql<AttributeDefinitionRow[]>`
    ...
    where pad.provider_type_id = ${id}::uuid
  ...
  const optionRows = attributeIds.length
    ? await sql<AttributeOptionRow[]>`
        ...
        where provider_attribute_definition_id = any(${sql.array(attributeIds)}::uuid[])
        order by id asc
      `
    : [];
...
export async function updateProviderType(input: ProviderTypeInput & { providerTypeId?: string }): Promise<boolean> {
  ...
  const rows = await sql<{ id: string }[]>`
    ...
    where id = ${input.providerTypeId}::uuid
    returning id
  `;
  ...
  return sql.begin(async (tx) => {
    const rows = await tx<{ id: string }[]>`
      ...
      set
        ...
        last_modified_date = now()
      where id = ${attributeDefinitionId}::uuid
        and provider_type_id = ${providerTypeId}::uuid
      returning id
    `;
    ...
    await tx`
      ...
      where provider_attribute_definition_id = ${attributeDefinitionId}::uuid
    `;
