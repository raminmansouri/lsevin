...
export async function getPatientTimeline(
  ...
  const rows = await db<any[]>`
    ...
    -- View/download/access/export events are still recorded in
    -- patient.audit_log (spec V3.1/V5.6/V7.3's logging requirements) but
    -- are access-log noise, not the kind of operational event the patient
    -- timeline (spec V1.2) is meant to show.
    and action not in (
      'document_viewed', 'document_downloaded', 'share_grant_viewed', 'share_grant_denied',
      'fhir_resource_exported', 'fhir_bundle_exported'
    )
