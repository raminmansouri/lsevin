...
import { getTrustedProvidersTag, getServiceProviderIdTag, getOffersTag } from "../../db/cache";
import { ITrendingServiceResponse, OffersResponse, TrustedProvider } from "../../types";
...
interface OffersFilterParams extends FilterParams {
}
...
const fetchOffers = async (
  ...
  console.log('fetch Offers:',filters)
  ...
  const path = `/customer/offers?${searchParams.toString()}`
  ...
  // const response = await readData<OffersResponse>(path);
...
const SERVICE_DEFINITION_DETAILS_CACHE_TAG = "fetch-Offers";
const queryKey = () =>
  [SERVICE_DEFINITION_DETAILS_CACHE_TAG] as const;

export const useFetchOffers = (filters: { priceRange: number[]; distance: number; minRating: number; verifiedOnly: boolean; languages: string[]; responseTime: "any" | "fast" | "instant"; }) => {
  const options = queryOptions<OffersResponse, IProblem>({
    queryKey: queryKey(),
    queryFn: () => fetchOffers(undefined, filters),
