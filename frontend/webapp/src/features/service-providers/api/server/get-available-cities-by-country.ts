...
export const getAvailableCitiesByCountry = async (
  request: BaseRequest,
  countryCode: string
): Promise<ApiReturnType<AvailableCity[]>> => {
  "use cache: remote";
  cacheLife("default");
  cacheTag(getServiceProviderGlobalTag());

  return readData<AvailableCity[]>(
    `${CATEGORY_MODULE_BASE_PATH}/service-providers/available-cities/${countryCode}`,
    { ...request }
  );
};
