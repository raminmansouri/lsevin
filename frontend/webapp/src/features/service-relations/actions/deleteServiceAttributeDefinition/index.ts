...
const schema = serviceAttributeDefinitionSchema.pick({ serviceDefinitionId: true, id: true });
...
const handler = async (input: InputType, token: string, _userId: string, locale: LocaleHeaderTypes): Promise<ReturnType> => {
  const { data, error } = await deleteData<unknown>(`${CATEGORY_MODULE_BASE_PATH}/${ADMIN_BASE_PATH}/service-definitions/${input.serviceDefinitionId}/attribute-definitions/${input.id}`, { token, locale });
  return { data, error };
};

export const deleteServiceAttributeDefinitionAction = createAuthenticatedSafeAction(schema, handler);
