...
const schema = providerServiceAddonSchema.pick({ providerServiceId: true, addonId: true });
...
const handler = async (input: InputType, token: string, _userId: string, locale: LocaleHeaderTypes): Promise<ReturnType> => {
  const { data, error } = await deleteData<unknown>(`${CATEGORY_MODULE_BASE_PATH}/${ADMIN_BASE_PATH}/provider-services/${input.providerServiceId}/addons/${input.addonId}`, { token, locale });
  return { data, error };
};

export const removeProviderServiceAddonAction = createAuthenticatedSafeAction(schema, handler);
