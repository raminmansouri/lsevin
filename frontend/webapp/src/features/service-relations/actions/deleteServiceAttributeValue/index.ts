...
const schema = serviceAttributeValueSchema.pick({ providerServiceId: true, id: true });
...
const handler = async (input: InputType, token: string, _userId: string, locale: LocaleHeaderTypes): Promise<ReturnType> => {
  const { data, error } = await deleteData<unknown>(`${CATEGORY_MODULE_BASE_PATH}/${ADMIN_BASE_PATH}/provider-services/${input.providerServiceId}/attribute-values/${input.id}`, { token, locale });
  return { data, error };
};

export const deleteServiceAttributeValueAction = createAuthenticatedSafeAction(schema, handler);
