...
const schema = serviceIncludedSchema.pick({ providerServiceId: true, id: true });
...
const handler = async (input: InputType, token: string, _userId: string, locale: LocaleHeaderTypes): Promise<ReturnType> => {
  const { data, error } = await deleteData<unknown>(`${CATEGORY_MODULE_BASE_PATH}/${ADMIN_BASE_PATH}/provider-services/${input.providerServiceId}/included/${input.id}`, { token, locale });
  return { data, error };
};

export const deleteServiceIncludedAction = createAuthenticatedSafeAction(schema, handler);
