...
import { upsertServiceFaq } from "../../server/repository";

type InputType = z.infer<typeof serviceFaqSchema>;
...
const handler = async (input: InputType, _token: string, _userId: string, locale: LocaleHeaderTypes): Promise<ReturnType> => {
  try {
    const data = await upsertServiceFaq(input);
...
export const upsertServiceFaqAction = createAuthenticatedSafeAction(serviceFaqSchema, handler);
