...
const schema = addonProviderTypeSchema.pick({ serviceDefinitionId: true, id: true });
...
const handler = async (input: InputType, token: string, _userId: string, locale: LocaleHeaderTypes): Promise<ReturnType> => {
  const { data, error } = await deleteData<unknown>(`${CATEGORY_MODULE_BASE_PATH}/${ADMIN_BASE_PATH}/service-definitions/${input.serviceDefinitionId}/addon-provider-types/${input.id}`, { token, locale });
  return { data, error };
};

export const deleteAddonProviderTypeAction = createAuthenticatedSafeAction(schema, handler);
