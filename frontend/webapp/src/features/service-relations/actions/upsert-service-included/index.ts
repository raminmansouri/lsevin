...
import { upsertServiceIncluded } from "../../server/repository";

type InputType = z.infer<typeof serviceIncludedSchema>;
...
const handler = async (input: InputType, _token: string, _userId: string, locale: LocaleHeaderTypes): Promise<ReturnType> => {
  try {
    const data = await upsertServiceIncluded(input);
...
export const upsertServiceIncludedAction = createAuthenticatedSafeAction(serviceIncludedSchema, handler);
