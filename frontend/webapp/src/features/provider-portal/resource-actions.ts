...
import { createProviderResourceRow, deleteProviderResourceRow, updateProviderResourceRow } from "./server/resource-repository";
...
export async function createProviderResourceAction(formData: FormData) {
  ...
  if (!providerId || !resource) throw new Error("Missing provider resource context.");
...
export async function updateProviderResourceAction(formData: FormData) {
  ...
  if (!providerId || !resource || !recordId) throw new Error("Missing provider resource context.");

  await updateProviderResourceRow(userId, providerId, resource, recordId, formData);
  revalidatePath(providerBase(locale, providerId));
  revalidatePath(`${providerBase(locale, providerId)}/manage/${resource}`);
  redirect(`${providerBase(locale, providerId)}/manage/${resource}`);
}
