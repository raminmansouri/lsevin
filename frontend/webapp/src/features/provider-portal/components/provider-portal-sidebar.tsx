...
import { tCommon, tResourceGroup, tResourceLabel, tStatus } from "../lib/i18n";
...
const topLinks = [
  ...
  },
  {
    ...
    label: "Profile",
    icon: Building2,
    permission: "manageProfile" as const,
  },
  {
    href: "services",
    label: "Services",
    icon: Sparkles,
    permission: "manageServices" as const,
  },
  {
    href: "staff",
    label: "Staff",
    icon: Users,
    permission: "manageStaff" as const,
  },
  {
    href: "availability",
    label: "Availability",
    icon: CalendarDays,
    permission: "manageAvailability" as const,
  },
  {
    href: "bookings",
    label: "Bookings",
  ...
  },
  {
    ...
    label: "Media",
    icon: ImageIcon,
    permission: "manageMedia" as const,
  },
  {
    href: "reviews",
    label: "Reviews",
    icon: MessageSquare,
    permission: "viewReviews" as const,
  },
  {
    href: "offers",
    label: "Offers",
    icon: Percent,
    permission: "manageOffers" as const,
  },
  {
    href: "billing",
    label: "Billing",
    icon: CreditCard,
    permission: "viewBilling" as const,
  },
  {
    href: "support",
    label: "Support",
    icon: LifeBuoy,
    permission: "manageSupport" as const,
  },
  {
    href: "settings",
    label: "Settings",
      ...
      <div className="flex h-full flex-col overflow-hidden bg-slate-950 text-white shadow-2xl lg:rounded-r-[2rem]">
        <div className="border-b border-white/10 p-6">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-white/45">
            {tCommon(t, "lsevinProviderAdmin", "LSevin Provider Admin")}
          </p>
          <h1 className="mt-3 line-clamp-2 text-2xl font-bold tracking-tight text-white">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <Badge
              variant={workspace.provider.isActive ? "default" : "secondary"}
            >
        ...
        <nav className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          <div className="mt-6 border-t border-white/10 pt-5">
            <p className="mb-3 px-3 text-[11px] font-semibold uppercase tracking-[0.22em] text-white/35">
              {tCommon(t, "crudPageData", "CRUD page data")}
            </p>
                ...
                return (
                  <div key={group}>
                    <div className="mb-1 flex items-center gap-2 px-3 text-xs font-semibold text-white/55">
                      ...
                      <span>{tResourceGroup(t, group)}</span>
                    </div>
                    <div className="space-y-1">
                      {visible.map((resource) => {
                        const Icon =
                          resourceIcons[resource.key] || ClipboardList;
        ...
        <div className="space-y-2 border-t border-white/10 bg-white/[0.03] p-4">
          <Link
            ...
            className="block rounded-2xl px-3 py-2 text-sm font-medium text-white/65 hover:bg-white/10 hover:text-white"
          >
            {tCommon(t, "switchProvider", "Switch provider")}
          </Link>
          <Link
            href="/provider-portal/applications"
            className="block rounded-2xl px-3 py-2 text-sm font-medium text-white/65 hover:bg-white/10 hover:text-white"
          >
            {tCommon(t, "myApplications", "My applications")}
          </Link>
        </div>
      </div>
    </aside>
