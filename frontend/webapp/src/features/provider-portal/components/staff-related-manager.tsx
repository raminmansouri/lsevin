...
export function StaffRelatedManager({
  ...
  const selectedStaff = staffId
    ? staff.filter(
        (item) => item.id === staffId || item.providerStaffId === staffId,
      )
    : staff;
  ...
  const scopedRelated: ProviderStaffRelatedRecords = staffId
    ? {
        ...related,
        certifications: related.certifications.filter((row) =>
          staffIds.has(row.staffId),
        ),
        education: related.education.filter((row) => staffIds.has(row.staffId)),
        availability: related.availability.filter((row) =>
          staffIds.has(row.staffId),
        ),
  ...
  return (
    <div className="space-y-6">
      {showCertifications || showEducation ? (
        <div
          className={
            focus === "all" ? "grid gap-6 xl:grid-cols-2" : "space-y-6"
          }
        >
          {showCertifications ? (
            <CertificationsCard
              ...
              rows={scopedRelated.certifications}
            />
          ) : null}
          {showEducation ? (
            <EducationCard
        ...
        >
          {showAvailability ? (
            <AvailabilityCard
              providerId={providerId}
              staff={effectiveStaff}
              rows={scopedRelated.availability}
            />
          ) : null}
          {showServices ? (
            <StaffServicesCard
...
function CertificationsCard({
  ...
  return (
    <Card className="rounded-3xl border-slate-200 shadow-sm">
      <CardHeader>
        <CardTitle>
          {tCommon(t, "staffCertifications", "Staff certifications")}
        </CardTitle>
        <CardDescription>
          {tCommon(
            t,
            "staffCertificationsDescription",
            "Provider can add non-verified certifications for their own staff.",
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        ...
        <RecordList
          ...
          empty={tCommon(
            t,
            "noStaffCertificationsYet",
            "No staff certifications yet.",
          )}
          render={(row) => (
            <div
              key={row.id}
            ...
            >
              <div>
                ...
                {row.isVerified ? (
                  <Badge>{tCommon(t, "verified", "Verified")}</Badge>
                ) : null}
              </div>
              {!row.isVerified ? (
                <RowActions
...
function CertificationForm({
  ...
  providerId: string;
  staff: StaffRow[];
  editing: StaffCertificationRow | null;
  onDone: () => void;
}) {
  const t = useTranslations("ProviderPortal");
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const values = useMemo<CertificationValues>(
    () => ({
      providerId,
  ...
  const form = useForm<CertificationValues>({
    resolver: zodResolver(saveStaffCertificationSchema),
    values,
  });
  return (
    <InlineForm
      onSubmit={form.handleSubmit((v) =>
        startTransition(async () => {
          const r = await saveStaffCertificationAction(v);
          if (!r.ok)
            return toast.error(
              r.error || tCommon(t, "couldNotSave", "Could not save."),
            );
          toast.success(
            tCommon(t, "certificationSaved", "Certification saved."),
          );
          onDone();
          router.refresh();
        }),
    ...
    >
      ...
      <Field label="Issuer">
        <Input {...form.register("issuer")} disabled={isPending} />
      </Field>
      <SubmitButtons
        editing={!!editing}
        pending={isPending}
...
function EducationCard({
  ...
  return (
    <Card className="rounded-3xl border-slate-200 shadow-sm">
      <CardHeader>
        <CardTitle>{tCommon(t, "staffEducation", "Staff education")}</CardTitle>
        <CardDescription>
          {tCommon(
            t,
            "staffEducationDescription",
            "Degrees and institutions for doctors, trainers, teachers, and specialists.",
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
...
function EducationForm({
  ...
  providerId: string;
  staff: StaffRow[];
  editing: StaffEducationRow | null;
  onDone: () => void;
}) {
  const t = useTranslations("ProviderPortal");
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const values = useMemo<EducationValues>(
    () => ({
      providerId,
      educationId: editing?.id || null,
      staffId: editing?.staffId || staff[0]?.id || "",
  ...
  const form = useForm<EducationValues>({
    resolver: zodResolver(saveStaffEducationSchema),
    values,
  });
  return (
    <InlineForm
      onSubmit={form.handleSubmit((v) =>
        startTransition(async () => {
          const r = await saveStaffEducationAction(v);
          if (!r.ok)
            return toast.error(
              r.error || tCommon(t, "couldNotSave", "Could not save."),
            );
          toast.success(tCommon(t, "educationSaved", "Education saved."));
          onDone();
          router.refresh();
        }),
      )}
    >
      <input type="hidden" {...form.register("providerId")} />
      <input type="hidden" {...form.register("educationId")} />
      <StaffSelect
        form={form}
        staff={staff}
        disabled={isPending || !!editing}
      />
      <Field label="Degree">
        <Input {...form.register("degree")} disabled={isPending} />
      </Field>
      <Field label="Institution">
        <Input {...form.register("institution")} disabled={isPending} />
      </Field>
      <Field label="Year">
        <Input type="number" {...form.register("year")} disabled={isPending} />
      </Field>
      <SubmitButtons
        editing={!!editing}
        pending={isPending}
...
function AvailabilityCard({
  ...
  return (
    <Card className="rounded-3xl border-slate-200 shadow-sm">
      <CardHeader>
        <CardTitle>
          {tCommon(t, "staffAvailability", "Staff availability")}
        </CardTitle>
        <CardDescription>
          {tCommon(
            t,
            "staffAvailabilityDescription",
            "Detailed staff-level availability on top of provider operating hours.",
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        ...
        <RecordList
          ...
          empty={tCommon(
            t,
            "noStaffAvailabilityRecordsYet",
            "No staff availability records yet.",
          )}
          render={(row) => (
            <div
              key={row.id}
            ...
            >
              <div>
                ...
                <p className="text-sm text-slate-500">
                  {dayNames[row.dayOfWeek - 1]} · {row.startTime} -{" "}
                  {row.endTime}
                </p>
              </div>
              <RowActions
...
function AvailabilityForm({
  ...
  const cleanTime = (value?: string | null) =>
    (value || "09:00").split(":").slice(0, 2).join(":");
  const values = useMemo<AvailabilityValues>(
    () => ({
      providerId,
      availabilityId: editing?.id || null,
      staffId: editing?.staffId || staff[0]?.id || "",
      dayOfWeek: editing?.dayOfWeek || 1,
      startTime: cleanTime(editing?.startTime),
      endTime: cleanTime(editing?.endTime || "18:00"),
      isRecurring: editing?.isRecurring ?? true,
      availabilityStatusId: editing?.availabilityStatusId || 1,
      specificDate: editing?.specificDate || null,
    }),
    [editing, providerId, staff],
  );
  const form = useForm<AvailabilityValues>({
    resolver: zodResolver(saveStaffAvailabilitySchema),
    values,
  });
  return (
    <InlineForm
      onSubmit={form.handleSubmit((v) =>
        startTransition(async () => {
          const r = await saveStaffAvailabilityAction(v);
          if (!r.ok)
            return toast.error(
              r.error || tCommon(t, "couldNotSave", "Could not save."),
            );
          toast.success(tCommon(t, "availabilitySaved", "Availability saved."));
          onDone();
          router.refresh();
        }),
      )}
    >
        ...
        >
          {dayNames.map((d, i) => (
            <option key={d} value={i + 1}>
              {tCommon(t, `days.${d.toLowerCase()}`, d)}
            </option>
          ))}
        </select>
      </Field>
      ...
      <Field label="End">
        <Input type="time" {...form.register("endTime")} disabled={isPending} />
      </Field>
      <label className="flex items-center gap-2 pb-3 text-sm">
        <input type="checkbox" {...form.register("isRecurring")} />{" "}
        {tCommon(t, "recurring", "Recurring")}
      </label>
      <SubmitButtons
        editing={!!editing}
        pending={isPending}
...
function StaffServicesCard({
  ...
  return (
    <Card className="rounded-3xl border-slate-200 shadow-sm">
      <CardHeader>
        <CardTitle>
          {tCommon(t, "staffServiceLinks", "Staff service links")}
        </CardTitle>
        <CardDescription>
          {tCommon(
            t,
            "staffServiceLinksDescription",
            "Choose which service definitions each staff member can provide.",
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        ...
        />
        <RecordList
          rows={related.services}
          empty={tCommon(
            t,
            "noStaffServiceLinksYet",
            "No staff service links yet.",
          )}
          render={(row) => (
            <div
              key={row.id}
            ...
            >
              <div>
                <p className="font-semibold">{row.serviceDefinitionName}</p>
                <p className="text-sm text-slate-500">{row.staffName}</p>
                <Badge variant={row.isActive ? "default" : "secondary"}>
                  {row.isActive ? tStatus(t, "active") : tStatus(t, "inactive")}
                </Badge>
              </div>
              <RowActions
                onEdit={() => setEditing(row)}
...
function StaffServiceForm({
  ...
  staff: StaffRow[];
  related: ProviderStaffRelatedRecords;
  editing: StaffServiceRow | null;
  onDone: () => void;
}) {
  const t = useTranslations("ProviderPortal");
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const values = useMemo<StaffServiceValues>(
    () => ({
      providerId,
      staffServiceId: editing?.id || null,
      staffId: editing?.staffId || staff[0]?.id || "",
      serviceDefinitionId:
        editing?.serviceDefinitionId || related.serviceDefinitions[0]?.id || "",
      notesEn: editing ? displayTranslation(editing.notes, "en-US", "") : "",
      notesFa: editing ? displayTranslation(editing.notes, "fa-IR", "") : "",
      isActive: editing?.isActive ?? true,
    }),
    [editing, providerId, related.serviceDefinitions, staff],
  );
  const form = useForm<StaffServiceValues>({
    resolver: zodResolver(saveStaffServiceSchema),
    values,
  });
  return (
    <InlineForm
      onSubmit={form.handleSubmit((v) =>
        startTransition(async () => {
          const r = await saveStaffServiceAction(v);
          if (!r.ok)
            return toast.error(
              r.error || tCommon(t, "couldNotSave", "Could not save."),
            );
          toast.success(
            tCommon(t, "staffServiceSaved", "Staff service saved."),
          );
          onDone();
          router.refresh();
        }),
    ...
    >
      <input type="hidden" {...form.register("providerId")} />
      <input type="hidden" {...form.register("staffServiceId")} />
      <StaffSelect
        form={form}
        staff={staff}
        disabled={isPending || !!editing}
      ...
      <Field label="Service definition">
        <select
          ...
          className="h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-sm"
        >
          {related.serviceDefinitions.map((s) => (
...
function StaffGalleryCard({
  ...
  return (
    <Card className="rounded-3xl border-slate-200 shadow-sm">
      <CardHeader>
        <CardTitle>{tCommon(t, "staffGallery", "Staff gallery")}</CardTitle>
        <CardDescription>
          {tCommon(
            t,
            "staffGalleryDescription",
            "Before/after, certificates, workplace images, videos, and specialist media.",
          )}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {rows.length ? (
              ...
              >
                <div className="flex gap-4">
                  <div className="relative h-20 w-24 shrink-0 overflow-hidden rounded-2xl border border-slate-200">
                    <PortalImage
                      src={row.url}
                      alt={row.displayTitle || row.staffName}
                    />
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold">
                      {row.displayTitle ||
                        tCommon(t, "untitledMedia", "Untitled media")}
                    </p>
                    <p className="text-sm text-slate-500">
                      {row.staffName} · {row.mediaType}
                    </p>
                    {row.isPrimary ? (
                      <Badge>{tCommon(t, "primary", "Primary")}</Badge>
                    ) : null}
                    <LocalizedRichPreview translations={row.description} />
                  </div>
                </div>
            ...
            <Empty
              text={tCommon(t, "noStaffMediaYet", "No staff media yet.")}
            />
          )}
        </div>
      </CardContent>
...
function StaffGalleryForm({
  ...
  providerId: string;
  staff: StaffRow[];
  editing: StaffGalleryRow | null;
  onDone: () => void;
}) {
  const t = useTranslations("ProviderPortal");
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const values = useMemo<GalleryValues>(
    () => ({
      providerId,
      staffGalleryItemId: editing?.id || null,
      staffId: editing?.staffId || staff[0]?.id || "",
  ...
  const form = useForm<GalleryValues>({
    resolver: zodResolver(saveStaffGalleryItemSchema),
    values,
  });
  return (
    <InlineForm
      onSubmit={form.handleSubmit((v) =>
        startTransition(async () => {
          const r = await saveStaffGalleryItemAction(v);
          if (!r.ok)
            return toast.error(
              r.error || tCommon(t, "couldNotSave", "Could not save."),
            );
          toast.success(tCommon(t, "staffMediaSaved", "Staff media saved."));
          onDone();
          router.refresh();
        }),
      )}
    >
      <input type="hidden" {...form.register("providerId")} />
      <input type="hidden" {...form.register("staffGalleryItemId")} />
      <StaffSelect
        form={form}
        staff={staff}
        disabled={isPending || !!editing}
      ...
      <Field label="Media URL / media id">
...
function InlineForm({
  ...
  return (
    <form
      onSubmit={onSubmit}
      className="grid gap-4 rounded-2xl border border-slate-100 bg-slate-50/60 p-4 md:grid-cols-2"
    >
      {children}
    </form>
  );
  ...
  return (
    <div className="flex gap-2">
      <Button size="sm" variant="outline" onClick={onEdit}>
        <Edit className="mr-2 h-4 w-4" /> {tCommon(t, "edit", "Edit")}
      </Button>
      <Button
        size="sm"
        variant="destructive"
        disabled={pending}
        onClick={() => {
          if (!confirm(tCommon(t, "deleteThisRecord", "Delete this record?")))
            return;
          startTransition(async () => {
            const r = await onDelete();
            if (!r.ok)
              return toast.error(
                r.error || tCommon(t, "couldNotDelete", "Could not delete."),
              );
            toast.success(tCommon(t, "recordDeleted", "Record deleted."));
            router.refresh();
          });
        }}
      >
