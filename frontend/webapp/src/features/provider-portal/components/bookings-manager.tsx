...
export function BookingsManager({ workspace, bookings }: { workspace: ProviderWorkspace; bookings: BookingRow[] }) {
  const [editing, setEditing] = useState<BookingRow | null>(null);

  return (
    <div className="space-y-6">
      {editing ? (
        <BookingUpdateForm providerId={workspace.provider.id} booking={editing} onDone={() => setEditing(null)} />
      ) : null}
      ...
      <Card className="rounded-3xl border-slate-200 shadow-sm">
        <CardHeader>
          ...
          <CardDescription>Main bookings and child bookings assigned to this provider.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          {bookings.length ? bookings.map((booking) => (
            <div key={`${booking.bookingSource}-${booking.id}`} className="rounded-2xl border border-slate-200 p-4">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-600">
                    Customer: {booking.customerName || "-"} {booking.customerEmail ? `(${booking.customerEmail})` : ""}
                  </p>
                  <p className="mt-1 text-sm text-slate-600">
                    Total: {booking.currencyCode || ""} {booking.totalAmount.toLocaleString()}
                  </p>
                  {booking.providerNotes ? <p className="mt-2 rounded-xl bg-slate-50 p-3 text-sm text-slate-600">{booking.providerNotes}</p> : null}
                </div>
                {workspace.permissions.manageBookings ? (
            ...
            <div className="rounded-2xl border border-dashed p-8 text-center text-sm text-slate-500">No bookings yet.</div>
          )}
        </CardContent>
      </Card>
    </div>
...
function BookingUpdateForm({ providerId, booking, onDone }: { providerId: string; booking: BookingRow; onDone: () => void }) {
      ...
      <CardContent>
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4">
          ...
          <div className="flex justify-end gap-3 border-t pt-5">
            <Button type="button" variant="outline" onClick={onDone} disabled={isPending}>Cancel</Button>
