...
} from "@/features/booking-v2/lib/types";
...
export function BookingWizard({
  ...
  const selectedProvider = useMemo(
    () => catalog.providers.find((item) => item.id === values.providerId),
    [catalog.providers, values.providerId],
  );
  const selectedService = useMemo(
    () => catalog.services.find((item) => item.id === values.serviceId),
    [catalog.services, values.serviceId],
  );
  const selectedSpecialist = useMemo(
    () => catalog.specialists.find((item) => item.id === values.specialistId),
    [catalog.specialists, values.specialistId],
  );
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      ...
      if (values.providerId) query.set("providerId", values.providerId);
      if (values.serviceId) query.set("serviceId", values.serviceId);
      if (values.specialistId) query.set("specialistId", values.specialistId);
  ...
  const loadPendingDrafts = useCallback(async () => {
    try {
      const result = await fetchJson<{ drafts: any[] }>("/api/booking-v2/drafts");
  ...
  const loadAddons = useCallback(async () => {
    ...
    try {
      ...
      const result = await fetchJson<{ addons: BookingAddon[] }>(`/api/booking-v2/addons?${query.toString()}`);
  ...
  const loadDates = useCallback(async () => {
    ...
    try {
      const result = await fetchJson<{ dates: AvailabilityDate[] }>(`/api/booking-v2/availability/dates?specialistId=${values.specialistId}`);
  ...
  const loadTimes = useCallback(async () => {
    ...
    try {
      ...
      const result = await fetchJson<{ times: AvailabilityTime[] }>(`/api/booking-v2/availability/times?${query.toString()}`);
  ...
  const saveDraft = useCallback(async (targetStep = step) => {
    ...
    try {
      ...
      const result = await fetchJson<{ draft: BookingDraftRecord }>(url, {
        method,
  ...
  const hydrateDraft = useCallback(async (draftId: string) => {
    try {
      const result = await fetchJson<{ draft: BookingDraftRecord }>(`/api/booking-v2/drafts/${draftId}`);
  ...
  const canContinue = () => {
    if (step === 1) return Boolean(values.providerId && values.serviceId && values.specialistId);
    if (step === 2) return Boolean(values.selectedDate && values.selectedTime && values.selectedTimeFrom && values.selectedTimeTo);
    if (step === 3) return true;
    if (step === 4) return true;
    if (step === 5) return Boolean(values.paymentMethod);
    return false;
  };
  ...
  const toggleAddon = (addon: BookingAddon) => {
    ...
    if (exists) {
      methods.setValue(
        "selectedAddons",
        current.filter((item) => item.addonId !== addon.id),
        { shouldDirty: true },
      );
      return;
    }
  ...
  const submitCheckout = methods.handleSubmit(async (formValues) => {
    ...
    try {
      ...
      if (!draft?.id) {
        throw new Error("Draft booking could not be saved before checkout.");
      }
    ...
    return (
      <div className="mx-auto max-w-4xl px-4 py-8">
        <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-8 text-center">
          ...
          <h1 className="mt-4 text-2xl font-bold text-slate-900">Booking created</h1>
          <p className="mt-2 text-slate-600">Your booking and payment record were saved to the database.</p>
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        <div className="mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-600">Standalone booking process with draft persistence and checkout.</p>
          </div>
        </div>
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[1.7fr_0.8fr]">
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {loading.catalog ? (
              <div className="flex min-h-[240px] items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-slate-400" /></div>
            ) : null}

            {!loading.catalog && step === 1 ? (
              <section className="space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-slate-900">Choose provider, service, and specialist</h2>
                  <p className="mt-1 text-sm text-slate-600">This step creates the base booking context.</p>
                </div>

                <SelectionGrid
                  title="Provider"
                  icon={<Building2 className="h-4 w-4" />}
                  items={catalog.providers.map((item) => ({ id: item.id, title: item.name, subtitle: [item.city, item.country].filter(Boolean).join(", ") }))}
                ...
                <SelectionGrid
                  ...
                  items={catalog.services
                    ...
                    .map((item) => ({ id: item.id, title: item.name, subtitle: `${item.currency} ${item.price} • ${item.durationMinutes} min` }))}
                ...
                <SelectionGrid
                  ...
                  items={catalog.specialists.map((item) => ({ id: item.id, title: item.name, subtitle: item.title || "Specialist" }))}
                  value={values.specialistId}
                  onSelect={(value) => methods.setValue("specialistId", value, { shouldDirty: true })}
                />
              </section>
            ) : null}

            {!loading.catalog && step === 2 ? (
              <section className="space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-slate-900">Schedule appointment</h2>
                  <p className="mt-1 text-sm text-slate-600">Available dates and times come from staff availability and existing bookings.</p>
                </div>

                <div>
                  <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-slate-900"><Calendar className="h-4 w-4" /> Select date</div>
                ...
                <div>
                  <div className="mb-3 flex items-center gap-2 text-sm font-semibold text-slate-900"><Clock3 className="h-4 w-4" /> Select time</div>
                  <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
                    {loading.times ? <Loader2 className="h-5 w-5 animate-spin text-slate-400" /> : null}
                    {times.map((time) => (
                      <button
                        type="button"
                        key={`${time.timeFrom}-${time.timeTo}`}
              ...
              <section className="space-y-6">
                <div className="flex items-start justify-between gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">Provider add-ons always appear. Turn on LSevin add-ons to show hotel, pickup, and other travel help.</p>
                  </div>
                  <label className="inline-flex items-center gap-3 text-sm font-medium text-slate-900">
                    <input
                      type="checkbox"
                      checked={values.useLsevin}
                      onChange={(event) => methods.setValue("useLsevin", event.target.checked, { shouldDirty: true })}
                      className="h-4 w-4 rounded border-slate-300"
                    />
                    I want to use LSevin
                  </label>
                </div>
                ...
                <div className="space-y-4">
                  {addons.map((addon) => {
                    ...
                    return (
                      <div key={addon.id} className={`rounded-2xl border p-4 ${selected ? "border-slate-900 bg-slate-50" : "border-slate-200 bg-white"}`}>
                        ...
                        {selected && addon.addonKind === "hotel" ? (
                        ...
                        {selected && addon.addonKind === "airport_pickup" ? (
                          <div className="mt-4 grid gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-2">
            ...
            {!loading.catalog && step === 4 ? (
              <section className="space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-slate-900">Documents</h2>
                  <p className="mt-1 text-sm text-slate-600">Upload medical files or travel documents. Files are attached to the draft immediately.</p>
                </div>
                <div className="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5">
                  <label className="flex cursor-pointer flex-col items-center justify-center gap-2 text-center">
                    <Upload className="h-6 w-6 text-slate-500" />
                    <span className="text-sm font-medium text-slate-900">Click to upload a document</span>
                ...
                <div className="space-y-3">
                  {(values.documents || []).map((document, index) => (
                    <div key={`${document.fileUrl}-${index}`} className="flex items-center justify-between rounded-2xl border border-slate-200 bg-white p-4">
                      <div>
                      ...
                      <button type="button" onClick={() => removeDocument(index)} className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700">Remove</button>
                    </div>
                  ))}
                </div>
              </section>
            ...
            {!loading.catalog && step === 5 ? (
              <section className="space-y-6">
                <div>
                  <h2 className="text-xl font-bold text-slate-900">Review & payment</h2>
                  <p className="mt-1 text-sm text-slate-600">Final review before creating the booking and payment record.</p>
                </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="mb-3 text-sm font-semibold text-slate-900">Payment method</div>
                  <div className="grid gap-3 md:grid-cols-3">
                    <PaymentChoice value={values.paymentMethod} current="manual_transfer" title="Manual transfer" icon={<Landmark className="h-4 w-4" />} onSelect={(value) => methods.setValue("paymentMethod", value)} />
                    <PaymentChoice value={values.paymentMethod} current="online_gateway" title="Online gateway" icon={<CreditCard className="h-4 w-4" />} onSelect={(value) => methods.setValue("paymentMethod", value)} />
                    <PaymentChoice value={values.paymentMethod} current="pay_on_arrival" title="Pay on arrival" icon={<Shield className="h-4 w-4" />} onSelect={(value) => methods.setValue("paymentMethod", value)} />
                  </div>
                </div>
                ...
                <button type="button" onClick={submitCheckout} disabled={loading.submit} className="flex h-14 w-full items-center justify-center gap-2 rounded-2xl bg-slate-900 text-sm font-bold text-white transition hover:bg-slate-800 disabled:opacity-60">
                  {loading.submit ? <Loader2 className="h-4 w-4 animate-spin" /> : <CreditCard className="h-4 w-4" />}
            ...
            <div className="mt-8 flex items-center justify-between gap-3 border-t border-slate-200 pt-5">
              <button type="button" onClick={goBack} className="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">Back</button>
              <div className="flex items-center gap-3">
                <button type="button" onClick={() => saveDraft(step)} className="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-700 hover:bg-slate-50">Save draft</button>
                {step < 5 ? (
                  <button type="button" disabled={!canContinue()} onClick={goNext} className="flex items-center gap-2 rounded-2xl bg-slate-900 px-5 py-3 text-sm font-semibold text-white hover:bg-slate-800 disabled:opacity-60">
                    Continue
          ...
          <aside className="h-fit rounded-3xl border border-slate-200 bg-white p-5 shadow-sm lg:sticky lg:top-6">
            ...
            <div className="mt-5 border-t border-slate-200 pt-4">
              <div className="mb-3 text-sm font-semibold text-slate-900">Selected add-ons</div>
              <div className="space-y-2">
                {(values.selectedAddons || []).length === 0 ? (
                  <div className="text-sm text-slate-500">No add-ons selected.</div>
                ) : (
                  values.selectedAddons.map((addon) => {
                    const details = addons.find((item) => item.id === addon.addonId);
                    return (
                      <div key={addon.addonId} className="rounded-xl bg-slate-50 p-3">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {items.map((item) => (
          <button
            ...
            className={`rounded-2xl border p-4 text-left transition ${value === item.id ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 bg-white hover:border-slate-300"}`}
          >
            <div className="text-sm font-bold">{item.title}</div>
            {item.subtitle ? <div className={`mt-1 text-xs ${value === item.id ? "text-slate-200" : "text-slate-500"}`}>{item.subtitle}</div> : null}
          </button>
        ))}
      </div>
    </div>
...
function PaymentChoice({
  ...
  return (
    <button type="button" onClick={() => onSelect(current)} className={`rounded-2xl border p-4 text-left ${selected ? "border-slate-900 bg-white" : "border-slate-200 bg-white"}`}>
