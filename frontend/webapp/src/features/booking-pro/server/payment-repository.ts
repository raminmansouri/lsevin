...
export async function createBookingPaymentIntent(params: {
  ...
  const [booking] = await db`
    select b.id, b.user_id, b.total_amount, b.currency_code, b.payment_status, b.booking_status,
           p.id as payment_id, p.gateway_payload
  ...
  if (params.paymentMethodCode === 'wallet') {
    ...
    const [intent] = await db`
      insert into customer.wallet_payment_intents (
        ...
        'succeeded',
        ${{
          source: 'booking-admin-payments-patch',
          bookingId: params.bookingId,
          paymentId,
        } as any}
      )
  ...
  await db`
    update booking.payments
    set payment_method = ${params.paymentMethodCode},
        gateway = ${method.provider ?? params.paymentMethodCode},
        status = 'Pending',
        external_reference = ${externalReference},
        gateway_payload = coalesce(gateway_payload, '{}'::jsonb) || ${{
          actionUrl,
  ...
  return {
    ...
    externalReference,
    actionUrl,
    note: 'This is a pluggable adapter point. Replace actionUrl creation with your real gateway implementation.',
  };
}
    ...
    if (normalized === 'Succeeded') {
      ...
      await tx`
        ...
        set payment_status = 'Paid',
            paid_amount = coalesce(${booking?.total_amount ?? 0}, paid_amount),
