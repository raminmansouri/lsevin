...
export function EntityCard({ title, subtitle, description, imageUrl, selected, featured, meta = [], onClick }: Props) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group w-full overflow-hidden rounded-3xl border text-left transition-all ${selected ? 'border-[#083f30] bg-[#083f30]/5 shadow-xl ring-2 ring-[#083f30]/10' : 'border-slate-200 bg-white hover:-translate-y-0.5 hover:border-[#083f30]/30 hover:shadow-lg'}`}
    >
      <div className="relative h-36 w-full overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200">
        ...
        <div className="absolute left-4 top-4 flex items-center gap-2">
          {featured ? <span className="rounded-full bg-[#eacb7f] px-3 py-1 text-xs font-bold text-[#083f30]">Featured</span> : null}
          {selected ? <span className="rounded-full bg-[#083f30] px-3 py-1 text-xs font-bold text-white">Selected</span> : null}
        </div>
      </div>
      <div className="space-y-3 p-5">
        <div>
          ...
          {description ? <div className="mt-2 line-clamp-2 text-sm text-slate-600">{description}</div> : null}
        </div>
        {meta.length ? (
...
export function providerMeta(input: {
  ...
  return [
    ...
    input.rating ? { icon: <Star className="h-3.5 w-3.5" />, label: `${input.rating} stars${input.reviewCount ? ` · ${input.reviewCount}` : ''}` } : null,
    input.successRate ? { icon: <BadgeCheck className="h-3.5 w-3.5" />, label: `${input.successRate} success` } : null,
    input.totalPatients ? { icon: <Users className="h-3.5 w-3.5" />, label: `${input.totalPatients} patients` } : null,
...
export function serviceMeta(input: {
  ...
  return [
    input.rating ? { icon: <Star className="h-3.5 w-3.5" />, label: `${input.rating} stars${input.reviewCount ? ` · ${input.reviewCount}` : ''}` } : null,
    input.durationMinutes ? { icon: <Clock3 className="h-3.5 w-3.5" />, label: `${input.durationMinutes} min` } : null,
