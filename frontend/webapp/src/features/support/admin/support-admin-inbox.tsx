        ...
        <aside className="border-b lg:min-h-0 lg:border-b-0 lg:border-r bg-slate-50/70">
          <div className="border-b bg-white p-4">
            <div className="mb-4 flex items-center justify-between">
              <div>
                <h1 className="text-lg font-bold">{tAdmin("supportInbox")}</h1>
                <p className="text-xs text-muted-foreground">{tAdmin("crispLikeConversations")}</p>
              </div>
              {isPending && <Loader2 className="h-4 w-4 animate-spin text-muted-foreground" />}
            </div>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => event.key === "Enter" && refreshList()} placeholder={tAdmin("searchConversations")} className="rounded-2xl pl-9" />
            </div>
          ...
          {selected ? (
            ...
            <div className="flex flex-1 items-center justify-center p-8 text-center">
              <div>
                ...
                <h2 className="text-lg font-semibold">{tAdmin("selectAConversation")}</h2>
                <p className="mt-1 text-sm text-muted-foreground">{tAdmin("chooseAnItemFromTheInboxToStartReplying")}</p>
              </div>
            </div>
          )}
        </main>
        ...
        <aside className="hidden min-h-0 overflow-y-auto border-l bg-white p-4 lg:block">
          {selected ? (
            <div className="space-y-4">
              <Card className="rounded-3xl">
                <CardContent className="p-4">
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                    <div className="rounded-2xl bg-slate-50 p-3"><p className="text-muted-foreground">{tAdmin("source")}</p><p className="font-semibold">{selected.source}</p></div>
                    <div className="rounded-2xl bg-slate-50 p-3"><p className="text-muted-foreground">{tAdmin("locale")}</p><p className="font-semibold">{selected.locale || "-"}</p></div>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card className="rounded-3xl">
                <CardContent className="space-y-3 p-4">
                  <div className="flex items-center justify-between"><h3 className="font-semibold">{tAdmin("tags")}</h3><Tag className="h-4 w-4 text-muted-foreground" /></div>
              ...
              <Card className="rounded-3xl">
                <CardContent className="space-y-3 p-4">
                  <div className="flex items-center justify-between"><h3 className="font-semibold">{tAdmin("internalNote")}</h3><NotebookPen className="h-4 w-4 text-muted-foreground" /></div>
                  <Textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder={tAdmin("onlyAdminsCanSeeThisNote")} className="min-h-[100px] rounded-2xl" />
                  <Button type="button" variant="outline" onClick={addNote} disabled={!note.trim() || isPending} className="w-full rounded-2xl"><Plus className="mr-2 h-4 w-4" />{tAdmin("addNote")}</Button>
                </CardContent>
              </Card>
            ...
            <div className="flex h-full items-center justify-center text-center text-sm text-muted-foreground"><Headphones className="mr-2 h-4 w-4" />{tAdmin("customerContextAppearsHere")}</div>
          )}
        </aside>
      </div>
    </div>
