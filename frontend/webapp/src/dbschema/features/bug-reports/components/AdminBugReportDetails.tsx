...
import { AlertTriangle, ArrowLeft, Bug, CheckCircle2, FileText, Loader2, Lock, MessageCircle, Paperclip, Send } from "lucide-react";
...
import { addAdminBugReportMessageAction, archiveBugReportAction, updateBugReportStatusAction } from "../actions";
import { BUG_REPORT_STATUSES, BugReportAttachment, BugReportDetails as BugReportDetailsType, BugReportStatus } from "../types";
...
function AttachmentGrid({ attachments }: { attachments: BugReportAttachment[] }) {
  ...
  return (
    <div className="mt-3 grid grid-cols-2 gap-3 md:grid-cols-3">
      {attachments.map((attachment, index) => {
        ...
        return (
          <a key={`${attachment.fileUrl}-${index}`} href={url} target="_blank" rel="noreferrer" className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            {attachment.mediaType === "image" ? (
              ...
              <div className="flex h-36 flex-col items-center justify-center gap-2 bg-slate-50 text-slate-600">
                <FileText className="h-6 w-6" />
                <span className="max-w-[180px] truncate text-xs">{attachment.fileName}</span>
              </div>
            )}
            <div className="truncate px-3 py-2 text-xs font-medium text-slate-700">{attachment.fileName}</div>
          </a>
        );
      })}
    </div>
...
export function AdminBugReportDetails({ report }: Props) {
        ...
        <div className="grid gap-5 lg:grid-cols-[1fr_360px]">
          <section className="space-y-5">
            <div className="rounded-[32px] border border-slate-200 bg-white p-5 shadow-sm">
              ...
              <AttachmentGrid attachments={report.media} />
            </div>
                  ...
                  <div key={msg.id} className={`rounded-3xl border p-4 ${msg.isInternalNote ? "border-amber-200 bg-amber-50" : msg.senderType === "agent" ? "border-emerald-100 bg-emerald-50/60" : "border-slate-200 bg-slate-50"}`}>
                    <div className="mb-2 flex items-center justify-between gap-3 text-xs font-semibold text-slate-500">
                      <span className="inline-flex items-center gap-1">
                        ...
                        {msg.isInternalNote ? "Internal note" : msg.senderType}
                      </span>
