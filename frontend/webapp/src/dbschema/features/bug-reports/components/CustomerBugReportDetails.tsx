...
import { ArrowLeft, Bug, FileText, Loader2, MessageCircle, Paperclip, Send } from "lucide-react";
...
import { BugReportAttachment, BugReportDetails as BugReportDetailsType } from "../types";
        ...
        return (
          <a key={`${attachment.fileUrl}-${index}`} href={url} target="_blank" rel="noreferrer" className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            {attachment.mediaType === "image" ? (
              ...
              <div className="flex h-32 flex-col items-center justify-center gap-2 bg-slate-50 text-slate-600">
                <FileText className="h-6 w-6" />
                <span className="max-w-[160px] truncate text-xs">{attachment.fileName}</span>
              </div>
            )}
            ...
            {report.messages.map((msg) => (
              <div key={msg.id} className={`rounded-3xl border p-4 ${msg.senderType === "agent" ? "border-emerald-100 bg-emerald-50/70" : "border-slate-200 bg-slate-50"}`}>
                <div className="mb-2 flex items-center justify-between gap-3 text-xs font-semibold text-slate-500">
                  <span>{msg.senderType === "agent" ? "LSevin support" : "You"}</span>
