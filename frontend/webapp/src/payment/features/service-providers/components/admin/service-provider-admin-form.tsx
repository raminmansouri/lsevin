...
export function ServiceProviderAdminForm({ provider, lookups, locale }: Props) {
  ...
  const selectedCountryLookup = useMemo(() => {
    const normalizedSelectedCountry = String(selectedCountry || "")
      .trim()
      .toLowerCase();
  ...
  }, [lookups.countries, selectedCountry]);
  ...
  }, [cityParentId, initialCityOptions, locale, selectedCountry]);
                          ...
                          <FormControl>
                            <LazyAdminLookupSelect
                              ...
                              onValueChange={(value) => {
                                ...
                                // Changing country invalidates the previous city, but do not
                                // validate the empty reset. Otherwise Zod shows the raw
                                // "Too small" error before the user can choose a city.
