...
import { getGetRewardsPage } from "@/features/customer/api/server/get-reward-page";

export async function GET(request: NextRequest) {
  const { searchParams } = new URL(request.url);
  console.log('GetRewardsPage called:',searchParams)
...
const responseTime:any= searchParams.get("responseTime");
  ...
  const { data, error } = await getGetRewardsPage(
