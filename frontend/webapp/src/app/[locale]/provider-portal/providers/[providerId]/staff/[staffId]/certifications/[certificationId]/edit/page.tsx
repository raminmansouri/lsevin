...
export default async function EditStaffCertificationPage({
  params,
}: {
  params: Promise<{
    locale: string;
  ...
  const item = related.certifications.find(
    (row) => row.id === certificationId && row.staffId === staffId,
  );
  if (!item) notFound();
  ...
  return (
    <ProviderRecordForm
      ...
      title="Edit staff certification"
      description="Update this staff certification."
      fields={fields}
      initialValues={{
        providerId,
