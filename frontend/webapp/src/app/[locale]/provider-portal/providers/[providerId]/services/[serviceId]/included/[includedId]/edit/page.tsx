...
export default async function EditServiceIncludedPage({
  params,
}: {
  params: Promise<{
    locale: string;
  ...
  const item = related.included.find(
    (row) => row.id === includedId && row.providerServiceId === serviceId,
  );
  if (!item) notFound();
  ...
  return (
    <ProviderRecordForm
      ...
      description="Update this included item."
      fields={fields}
      initialValues={{
        providerId,
        includedId: item.id,
      ...
      backHref={providerPortalBack(
        providerId,
        `/services/${serviceId}/included`,
      )}
      submitLabel="Save item"
      successMessage="Included item updated."
    />
  );
}
