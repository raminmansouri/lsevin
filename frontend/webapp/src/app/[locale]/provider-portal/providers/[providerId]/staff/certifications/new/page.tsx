...
export default async function NewStaffCertificationFromStaffPage({ params }: { params: Promise<{ locale: string; providerId: string }> }) {
  ...
  return <ProviderRecordForm operation="saveStaffCertification" title="Add staff certification" description="Choose a staff member and add a certification." fields={fields} initialValues={{ providerId, staffId: staff[0]?.id || "" }} backHref={providerPortalBack(providerId, "/staff/certifications")} submitLabel="Save certification" successMessage="Certification saved." />;
}
