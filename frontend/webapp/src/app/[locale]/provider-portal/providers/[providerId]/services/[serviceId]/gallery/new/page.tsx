...
export default async function NewServiceGalleryItemPage({ params }: { params: Promise<{ locale: string; providerId: string; serviceId: string }> }) {
  ...
  return <ProviderRecordForm operation="saveServiceGalleryItem" title="Add service gallery item" description="Add an image/video/GIF to this service." fields={fields} initialValues={{ providerId, providerServiceId: serviceId, mediaType: "image", displayOrder: 0, isPrimary: false }} backHref={providerPortalBack(providerId, `/services/${serviceId}/gallery`)} submitLabel="Save gallery item" successMessage="Gallery item saved." />;
}
