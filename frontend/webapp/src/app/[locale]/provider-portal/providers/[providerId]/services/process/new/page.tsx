...
export default async function NewServiceProcessFromServicesPage({
  params,
}: {
  ...
  const nextStep = defaultServiceId
    ? related.process.filter(
        (item) => item.providerServiceId === defaultServiceId,
      ).length + 1
    : 1;
  ...
  return (
    <ProviderRecordForm
      ...
      description="Choose a service and create a customer-facing process step."
      fields={fields}
      initialValues={{
        providerId,
        providerServiceId: defaultServiceId,
        step: nextStep,
      }}
      backHref={providerPortalBack(providerId, "/services/process")}
      submitLabel="Save step"
      successMessage="Process step saved."
    />
  );
}
