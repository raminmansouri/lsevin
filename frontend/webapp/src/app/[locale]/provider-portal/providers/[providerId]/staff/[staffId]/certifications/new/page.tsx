...
export default async function NewStaffCertificationPage({ params }: { params: Promise<{ locale: string; providerId: string; staffId: string }> }) {
  ...
  return <ProviderRecordForm operation="saveStaffCertification" title="Add staff certification" description="Add a certification to this staff/specialist profile." fields={fields} initialValues={{ providerId, staffId }} backHref={providerPortalBack(providerId, `/staff/${staffId}/certifications`)} submitLabel="Save certification" successMessage="Certification saved." />;
}
