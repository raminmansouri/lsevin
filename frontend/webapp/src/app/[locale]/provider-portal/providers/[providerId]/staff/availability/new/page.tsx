...
export default async function NewStaffAvailabilityFromStaffPage({ params }: { params: Promise<{ locale: string; providerId: string }> }) {
  ...
  return <ProviderRecordForm operation="saveStaffAvailability" title="Add staff availability" description="Choose a staff member and create a working time block." fields={fields} initialValues={{ providerId, staffId: staff[0]?.id || "", dayOfWeek: 1, startTime: "09:00", endTime: "17:00", availabilityStatusId: 1, isRecurring: true }} backHref={providerPortalBack(providerId, "/staff/availability")} submitLabel="Save availability" successMessage="Availability saved." />;
}
