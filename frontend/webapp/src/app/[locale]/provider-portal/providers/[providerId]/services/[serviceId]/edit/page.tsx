...
export default async function EditProviderServicePage({ params }: { params: Promise<{ locale: string; providerId: string; serviceId: string }> }) {
  ...
  return (
    <ProviderRecordForm
      ...
      fields={fields}
      initialValues={{ providerId, serviceId: service.id, serviceDefinitionId: service.serviceDefinitionId, nameEn: tr(service.displayName, "en-US"), nameFa: tr(service.displayName, "fa-IR"), descriptionEn: tr(service.description, "en-US"), descriptionFa: tr(service.description, "fa-IR"), currency: service.currency, value: service.value, durationMinutes: service.durationMinutes, slotIntervalMinutes: service.slotIntervalMinutes, imageUrl: service.imageUrl || "", isActive: service.isActive, isPopular: service.isPopular, tagsCsv: csv(service.tags) }}
