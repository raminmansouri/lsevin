...
export default async function EditServiceAddonPage({ params }: { params: Promise<{ locale: string; providerId: string; serviceId: string; addonId: string }> }) {
  ...
  const setting = related.addonSettings.find((row) => row.providerServiceId === serviceId && row.addonId === addonId);
  if (!setting) notFound();
  ...
  return <ProviderRecordForm operation="saveServiceAddonSetting" title="Edit service add-on" description="Update this service add-on setting." fields={fields} initialValues={{ providerId, providerServiceId: setting.providerServiceId, addonId: setting.addonId, customPrice: setting.customPrice || "", isEnabled: setting.isEnabled }} backHref={providerPortalBack(providerId, `/services/${serviceId}/add-ons`)} submitLabel="Save add-on" successMessage="Add-on updated." />;
}
