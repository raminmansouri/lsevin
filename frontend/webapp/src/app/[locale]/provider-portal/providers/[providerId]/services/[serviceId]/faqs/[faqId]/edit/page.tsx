...
export default async function EditServiceFaqPage({ params }: { params: Promise<{ locale: string; providerId: string; serviceId: string; faqId: string }> }) {
  ...
  const item = related.faqs.find((row) => row.id === faqId && row.providerServiceId === serviceId);
  if (!item) notFound();
  ...
  return <ProviderRecordForm operation="saveServiceFaq" title="Edit service FAQ" description="Update this service FAQ." fields={fields} initialValues={{ providerId, faqId: item.id, providerServiceId: item.providerServiceId, question: item.question || "", answer: item.answer || "" }} backHref={providerPortalBack(providerId, `/services/${serviceId}/faqs`)} submitLabel="Save FAQ" successMessage="FAQ updated." />;
}
