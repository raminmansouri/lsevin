...
export default async function EditStaffEducationPage({ params }: { params: Promise<{ locale: string; providerId: string; staffId: string; educationId: string }> }) {
  ...
  const item = related.education.find((row) => row.id === educationId && row.staffId === staffId);
  if (!item) notFound();
  ...
  return <ProviderRecordForm operation="saveStaffEducation" title="Edit staff education" description="Update this education record." fields={fields} initialValues={{ providerId, educationId: item.id, staffId: item.staffId, degree: item.degree, institution: item.institution, year: item.year || "" }} backHref={providerPortalBack(providerId, `/staff/${staffId}/education`)} submitLabel="Save education" successMessage="Education updated." />;
}
