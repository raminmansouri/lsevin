...
export default async function EditPayoutAccountPage({ params }: { params: Promise<{ locale: string; providerId: string; payoutAccountId: string }> }) {
  ...
  const account = billing.payoutAccounts.find((item) => item.id === payoutAccountId);
  if (!account) notFound();
  ...
  return <ProviderRecordForm operation="savePayoutAccount" title="Edit payout account" description="Update this payout account." fields={fields} initialValues={{ providerId, payoutAccountId: account.id, accountHolderName: account.accountHolderName, bankName: account.bankName || "", iban: account.iban || "", swiftCode: account.swiftCode || "", accountNumberLast4: account.accountNumberLast4 || "", country: account.country || "", currencyCode: account.currencyCode, isDefault: account.isDefault }} backHref={providerPortalBack(providerId, "/billing/payout-accounts")} submitLabel="Save payout account" successMessage="Payout account updated." />;
}
