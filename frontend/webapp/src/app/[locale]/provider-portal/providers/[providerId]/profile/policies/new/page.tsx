...
export default async function NewProviderPolicyPage({ params }: { params: Promise<{ locale: string; providerId: string }> }) {
  ...
  return <ProviderRecordForm operation="saveProviderPolicy" title="Add provider policy" description="Add cancellation, refund, booking, or visit policies shown on your provider page." fields={fields} initialValues={{ providerId, providerPolicyTypeId: related.policyTypes[0]?.id || "" }} backHref={providerPortalBack(providerId, "/profile/policies")} submitLabel="Save policy" successMessage="Policy saved." />;
}
