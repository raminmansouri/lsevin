...
export default async function EditProviderMediaPage({
  params,
}: {
  params: Promise<{
    locale: string;
  ...
  const item = (await listProviderGallery(userId, providerId, locale)).find(
    (row) => row.id === galleryItemId,
  );
  if (!item) notFound();
  ...
  return (
    <ProviderRecordForm
      operation="saveProviderGallery"
      title="Edit provider media"
      description="Update this provider gallery item."
      fields={fields}
      initialValues={{
        providerId,
      ...
      backHref={providerPortalBack(providerId, "/media")}
