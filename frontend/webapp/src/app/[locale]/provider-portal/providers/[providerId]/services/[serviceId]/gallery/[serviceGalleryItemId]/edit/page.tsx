...
import { addonOptions, providerPortalBack, providerServiceOptions, tr } from "@/features/provider-portal/lib/form-page-utils";

export default async function EditServiceGalleryItemPage({ params }: { params: Promise<{ locale: string; providerId: string; serviceId: string; serviceGalleryItemId: string }> }) {
  ...
  const item = related.serviceGallery.find((row) => row.id === serviceGalleryItemId && row.providerServiceId === serviceId);
  if (!item) notFound();
  ...
  return <ProviderRecordForm operation="saveServiceGalleryItem" title="Edit service gallery item" description="Update this service media item." fields={fields} initialValues={{ providerId, serviceGalleryItemId: item.id, providerServiceId: item.providerServiceId, titleEn: tr(item.title, "en-US"), titleFa: tr(item.title, "fa-IR"), descriptionEn: tr(item.description, "en-US"), descriptionFa: tr(item.description, "fa-IR"), url: item.url, mediaType: item.mediaType, displayOrder: item.displayOrder, isPrimary: item.isPrimary }} backHref={providerPortalBack(providerId, `/services/${serviceId}/gallery`)} submitLabel="Save gallery item" successMessage="Gallery item updated." />;
}
