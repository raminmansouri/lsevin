...
export default async function NewServiceIncludedFromServicesPage({ params }: { params: Promise<{ locale: string; providerId: string }> }) {
  ...
  return <ProviderRecordForm operation="saveServiceIncluded" title="Add included item" description="Choose a service and add one included package item." fields={fields} initialValues={{ providerId, providerServiceId: services[0]?.id || "" }} backHref={providerPortalBack(providerId, "/services/included")} submitLabel="Save item" successMessage="Included item saved." />;
}
