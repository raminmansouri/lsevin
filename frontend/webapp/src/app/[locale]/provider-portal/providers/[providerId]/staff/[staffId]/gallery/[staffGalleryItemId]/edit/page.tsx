...
export default async function EditStaffGalleryItemPage({
  params,
}: {
  params: Promise<{
    locale: string;
  ...
  const item = related.gallery.find(
    (row) => row.id === staffGalleryItemId && row.staffId === staffId,
  );
  if (!item) notFound();
  ...
  return (
    <ProviderRecordForm
      operation="saveStaffGalleryItem"
      title="Edit staff gallery item"
      description="Update this staff gallery item."
      fields={fields}
      initialValues={{
        providerId,
        staffGalleryItemId: item.id,
      ...
      backHref={providerPortalBack(providerId, `/staff/${staffId}/gallery`)}
