...
export default async function EditProviderOfferPage({ params }: { params: Promise<{ locale: string; providerId: string; offerId: string }> }) {
  ...
  return <ProviderRecordForm operation="saveOffer" title="Edit offer" description="Update this service offer." fields={fields} initialValues={{ providerId, offerId: offer.id, providerServiceId: offer.providerServiceId, title: offer.title, subtitle: offer.subtitle || "", discountPercent: offer.discountPercent, validUntil: toDateTimeLocal(offer.validUntil), code: offer.code || "", isActive: offer.isActive, isFeatured: offer.isFeatured, usageLimit: offer.usageLimit || "", descriptionEn: "", descriptionFa: "" }} backHref={providerPortalBack(providerId, "/offers")} submitLabel="Save offer" successMessage="Offer updated." />;
}
