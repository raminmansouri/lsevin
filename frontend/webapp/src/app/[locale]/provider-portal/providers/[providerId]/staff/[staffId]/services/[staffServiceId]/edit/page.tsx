...
export default async function EditStaffServicePage({
  params,
}: {
  params: Promise<{
    locale: string;
  ...
  const item = related.services.find(
    (row) => row.id === staffServiceId && row.staffId === staffId,
  );
  if (!item) notFound();
  ...
  return (
    <ProviderRecordForm
      ...
      title="Edit staff service link"
      description="Update this staff-to-service definition link."
      fields={fields}
      initialValues={{
        providerId,
        ...
        notesEn: tr(item.notes, "en-US"),
        notesFa: tr(item.notes, "fa-IR"),
        isActive: item.isActive,
      }}
      backHref={providerPortalBack(providerId, `/staff/${staffId}/services`)}
      submitLabel="Save service link"
      successMessage="Service link updated."
    />
  );
}
