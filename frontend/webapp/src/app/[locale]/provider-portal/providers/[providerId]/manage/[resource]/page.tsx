...
export default async function ProviderManageResourcePage({ params }: { params: Promise<{ locale: string; providerId: string; resource: string }> }) {
  const { locale, providerId, resource } = await params;
  const userId = await requireCurrentUserId();

  try {
    const { config, rows, role } = await listProviderResourceRows(userId, providerId, resource);
    return <ProviderResourceListPage locale={locale} providerId={providerId} config={config} rows={rows} role={role} />;
  } catch (error) {
