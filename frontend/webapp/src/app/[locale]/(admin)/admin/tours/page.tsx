...
import { ToursAdminBoard } from "@/features/tours/components/admin/tours-admin-board";
import { getTourDeparturesAdminPageData } from "@/features/tours/server/repository";
...
export default function AdminToursPage({ params }: Props) {
  return (
...
async function SuspenseBoundary({ params }: { params: Promise<LocaleParams> }) {
  ...
  const data = await getTourDeparturesAdminPageData(locale);

  // Migration 0042 has not run yet. Rendering the board here would query relations
  ...
  return <ToursAdminBoard data={data} />;
}
