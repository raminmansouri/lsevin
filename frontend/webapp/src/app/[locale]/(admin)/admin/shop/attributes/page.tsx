          ...
          {attributes.map((a: any) => (
            <section key={a.id} className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
              <div className="mb-2 flex items-center justify-between">
                <div>
                  ...
                  {a.is_variant_defining ? (
                    <span className="ms-1 rounded-full bg-emerald-100 px-2 py-0.5 text-xs text-emerald-700">variant-defining</span>
                  ) : null}
                  <span className="ms-2 text-xs text-gray-400">{a.product_count} product(s)</span>
                </div>
                <form action={deleteAttributeForm}>
                  <input type="hidden" name="id" value={a.id} />
                  <button className="text-xs text-red-600" disabled={a.product_count > 0}>delete</button>
                </form>
              </div>
              ...
              <div className="mb-2 flex flex-wrap gap-1.5">
                ...
                {!(a.values ?? []).length ? <span className="text-xs text-gray-400">No values yet.</span> : null}
              </div>
              ...
              <form action={addAttributeValueForm} className="flex flex-wrap items-center gap-1.5">
                ...
                <input name="value" placeholder="value (e.g. cotton)" required className="h-8 w-32 rounded border border-gray-300 px-2 text-xs" />
                <input name="label_en" placeholder="label EN" className="h-8 w-28 rounded border border-gray-300 px-2 text-xs" />
                <input name="label_fa" placeholder="برچسب" dir="rtl" className="h-8 w-24 rounded border border-gray-300 px-2 text-xs" />
                <input name="colorHex" placeholder="#hex" className="h-8 w-20 rounded border border-gray-300 px-2 text-xs" />
                <button className="h-8 rounded bg-[#083f30] px-3 text-xs font-semibold text-white">Add value</button>
              </form>
            </section>
          ))}
          {!attributes.length ? <p className="text-sm text-gray-400">No attributes defined yet.</p> : null}
        </div>

        <section className="h-fit rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
          <h2 className="mb-3 text-sm font-bold text-gray-900">New attribute</h2>
          <form action={upsertAttributeForm} className="space-y-2">
            <input name="name_en" placeholder="Name EN" required className={input} />
            <div className="grid grid-cols-2 gap-2">
              <input name="name_fa" placeholder="نام" dir="rtl" className={input} />
              <input name="name_ar" placeholder="الاسم" dir="rtl" className={input} />
            </div>
            <input name="slug" placeholder="slug (kebab-case)" required className={input} />
            <select name="displayType" className={input}>
              {DISPLAY_TYPES.map((d) => <option key={d} value={d}>{d}</option>)}
            </select>
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" name="isVariantDefining" /> variant-defining
            </label>
            <button className="w-full rounded bg-[#083f30] px-3 py-2 text-sm font-semibold text-white">Create attribute</button>
          </form>
        </section>
      </div>
    </div>
