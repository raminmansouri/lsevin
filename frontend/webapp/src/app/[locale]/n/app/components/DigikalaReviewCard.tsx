...
const TEXT = {
  en: {
    ...
    pendingReviewHint: "Only you can see this for now. It becomes public after admin approval.",
  },
  fa: {
    ...
    pros: "نقاط قوت",
    cons: "نقاط ضعف",
    helpfulPrompt: "آیا این خلاصه برایتان مفید بود؟",
    like: "پسندیدم",
    dislike: "نپسندیدم",
    verifiedBuyer: "خریدار / رزروکننده",
    ...
    pendingReview: "در انتظار تأیید",
    pendingReviewHint: "فعلاً فقط شما این نظر را می‌بینید. پس از تأیید ادمین برای همه نمایش داده می‌شود.",
  },
  ar: {
    showMore: "عرض المزيد",
    showLess: "عرض أقل",
    pros: "الإيجابيات",
    cons: "السلبيات",
    helpfulPrompt: "هل كان هذا الملخص مفيداً لك؟",
    like: "إعجاب",
    dislike: "عدم إعجاب",
    ...
    pendingReview: "في انتظار الموافقة",
    pendingReviewHint: "أنت فقط من يرى هذا الآن. سيصبح عاماً بعد موافقة الإدارة.",
  },
  tr: {
    ...
    pros: "Artılar",
    cons: "Eksiler",
    helpfulPrompt: "Bu özet sizin için yararlı oldu mu?",
    like: "Beğendim",
    dislike: "Beğenmedim",
    verifiedBuyer: "Rezervasyon yapan müşteri",
    adminReview: "Editoryal yorum",
    replies: "Yanıtlar",
    adminAnswer: "Admin yanıtı",
    customerAnswer: "Rezervasyon yapan müşteri yanıtı",
    writeReply: "Bu yorumu yanıtla",
    replyPlaceholder: "Yanıtınızı yazın...",
    submitReply: "Yanıt gönder",
    pendingReply: "Yanıtınız admin onayından sonra görünecek.",
    replyError: "Yalnızca bu öğeyi rezerve eden kullanıcılar yorumlara yanıt verebilir.",
    pendingReview: "Onay bekliyor",
    pendingReviewHint: "Şimdilik bunu yalnızca siz görüyorsunuz. Admin onayından sonra herkese açık olur.",
  },
} as const;

function labels(locale?: string | null) {
  const key = String(locale || "fa")
    .split("-")[0]
    ?.toLowerCase() as keyof typeof TEXT;
...
export function DigikalaReviewCard({
  ...
  return (
    <article className={`rounded-2xl border bg-white p-4 ${isPending ? "border-amber-300 ring-1 ring-amber-200" : "border-gray-200"} ${className || ""}`}>
      <div className="mb-3 flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex flex-wrap items-center gap-2">
            <h4 className="font-bold text-gray-900">{review.name || "LSevin customer"}</h4>
      ...
      {isPending ? (
        <div className="mt-3 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-700">
          {t.pendingReviewHint}
        </div>
      ) : (
