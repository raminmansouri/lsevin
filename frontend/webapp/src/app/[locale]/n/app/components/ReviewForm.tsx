...
function reviewText(locale?: string | null) {
  const key = String(locale || "fa").split("-")[0]?.toLowerCase() as keyof typeof REVIEW_FORM_TEXT;
...
export function getLocalizedReviewEligibilityMessage(locale?: string | null, reason?: ReviewEligibilityReason): string | null {
  ...
  const key = String(locale || "fa").split("-")[0]?.toLowerCase() as keyof typeof REVIEW_ELIGIBILITY_TEXT;
      ...
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-t-3xl bg-white animate-slide-up sm:rounded-3xl">
        {submitStatus === "success" ? (
          <div className="p-12 text-center">
            ...
            <h2 className="mb-2 text-2xl font-bold text-gray-900">{t("reviewSubmitted")}</h2>
            <p className="text-gray-600">{t("thankYouForSharingYourExperienceYourReview")}</p>
          </div>
        ) : (
          <>
            <div className="sticky top-0 z-10 rounded-t-3xl border-b border-gray-200 bg-white p-6 sm:rounded-t-3xl">
              <div className="flex items-start justify-between">
                <div><h2 className="mb-1 text-xl font-bold text-gray-900">{t("writeAReview")}</h2><p className="text-sm text-gray-600">{providerName}{treatmentName && ` • ${treatmentName}`}</p></div>
            ...
            <div className="space-y-6 p-6">
              ...
              <div>
                <label className="mb-3 block text-sm font-semibold text-gray-900">{t("overallRating")} <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="mb-2 block text-sm font-semibold text-gray-900">{t("treatmentTitleOptional")}</label>
                <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} placeholder={t("exampleHairTransplantDentalCareSpaDay")} maxLength={100} className="h-12 w-full rounded-xl border-2 border-gray-300 px-4 transition-colors focus:border-[#083f30] focus:outline-none" />
                <p className="mt-1 text-xs text-gray-500">{title.length}/100 characters</p>
              </div>

              <div>
                <label className="mb-2 block text-sm font-semibold text-gray-900">{t("yourReview")} <span className="text-red-500">*</span></label>
                <textarea value={comment} onChange={(e) => setComment(e.target.value)} placeholder={t("shareDetailsOfYourOwnExperienceAtThis")} rows={6} maxLength={1000} className={`w-full resize-none rounded-xl border-2 px-4 py-3 transition-colors focus:outline-none ${errors.comment ? "border-red-300 focus:border-red-500" : "border-gray-300 focus:border-[#083f30]"}`} />
              ...
              <div>
                <label className="mb-2 block text-sm font-semibold text-gray-900">{t("addPhotosOptional")}</label>
