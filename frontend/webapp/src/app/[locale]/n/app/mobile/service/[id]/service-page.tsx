    ...
    <section className="mb-8 rounded-3xl border border-gray-200 bg-white p-4 shadow-sm">
      <div className="mb-4 flex gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <LexicalDescription content={provider.description || data.service.providerDescription} className="mt-1 line-clamp-2 text-sm text-gray-600" />
        </div>
      </div>
      ...
      <div className="space-y-3">
        {policies.map((policy) => (
          <div key={policy.id} className="rounded-2xl border border-gray-200 bg-white p-4">
            ...
            <LexicalDescription content={policy.description} className="text-sm leading-relaxed text-gray-600" />
          </div>
        ))}
      </div>
    </section>
...
export default function ServicePage({ data, serviceId, locale }: ServicePageProps) {
  ...
  const handleReviewSubmit = async (review: ReviewFormSubmitValue) => {
    ...
    // The review is stored as pending and stays hidden from the public list until an
    // admin approves it. Optimistically show it to its author so the submission feels real.
    const payload = await response.json().catch(() => null);
    const created = payload?.review as TopReview | undefined;
    if (created?.id) {
      setVisibleReviews((current) => [
        { ...created, date: "", pending: true },
