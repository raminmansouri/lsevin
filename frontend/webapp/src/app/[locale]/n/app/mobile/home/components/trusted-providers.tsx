  ...
  if (!providers.length) {
    return (
      <div className="w-full rounded-3xl border border-dashed border-gray-200 bg-gray-50 px-5 py-8 text-center">
        <p className="text-sm font-semibold text-gray-900">No trusted providers yet</p>
        <p className="mt-1 text-xs text-gray-500">Verified and highly rated providers will appear here.</p>
      </div>
    );
  }
  ...
  return (
    <>
      {providers.map((provider) => (
        <TrustedProviderCard key={provider.id} provider={provider} />
      ))}
    </>
  );
}

function TrustedProviderCard({ provider }: { provider: HomeTrustedProvider }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-1">
        ...
        <span className="ml-0.5 text-xs text-gray-500">({provider.reviewCount.toLocaleString()})</span>
      </div>
    </Link>
  );
}
