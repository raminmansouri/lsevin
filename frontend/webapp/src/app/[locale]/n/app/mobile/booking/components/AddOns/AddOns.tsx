...
export const AddOns = () => {
    ...
    const { handleNext, handleBack, step, setStep, calculateTotal, addons, selectedAddons, services, providers, specialists, selectedDate, selectedTime, uploadedFiles, service, provider, selectedSpecialist, providerId, serviceId, specialistId, paymentMethod } = useBooking();
    const { data: addOnsResponse, refetch: refetchAddOns } = useGetAddons(providerId, serviceId, specialistId, locale);
    ...
    return (<>
        ...
        <div className="space-y-3">
            {addOnsResponse?.addons?.map(addon => (<div key={addon.id} className={`bg-white rounded-2xl border-2 transition-all overflow-hidden ${selectedAddons?.find(f => f.id == addon.id)
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-start justify-between mb-1">
                                    <div className="flex-1">
                                        ...
                                        <RichTextPreview content={addon.description} className="text-sm text-gray-600 mb-2"/>
                                    </div>
                                ...
                                <button onClick={() => toggleAddon(addon.id)} className={`w-full h-10 rounded-xl font-semibold transition-all ${selectedAddons?.find(f => f.id == addon.id)
                ? 'bg-[#083f30] text-white'
                : 'bg-gray-100 text-gray-900 hover:bg-gray-200'}`}>
                                    {selectedAddons?.find(f => f == addon.id) ? (<span className="flex items-center justify-center gap-2">
