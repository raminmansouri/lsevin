  ...
  if (!services.length) {
    return (
      <div className="w-full rounded-3xl border border-dashed border-gray-200 bg-gray-50 px-5 py-8 text-center">
        <p className="text-sm font-semibold text-gray-900">No trending services yet</p>
        <p className="mt-1 text-xs text-gray-500">Bookings and trending scores will populate this section.</p>
      </div>
    );
  }
...
function TrendingServiceCard({ service }: { service: HomeTrendingService }) {
      ...
      <div className="relative aspect-square">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3">
          ...
          <div className="flex items-center gap-1 text-white/80">
            <Users size={12} />
            <span className="text-xs font-medium">{service.bookingLabel} bookings</span>
          </div>
        </div>
      </div>
    </Link>
