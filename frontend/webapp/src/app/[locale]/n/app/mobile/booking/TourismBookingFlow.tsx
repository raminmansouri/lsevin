...
<<<<<<< HEAD
import { ArrowLeft, Calendar, Clock, ChevronRight, CheckCircle2, Plus, Shield, CreditCard, Building, Smartphone, BadgeCheck, Star, MapPin, Users, Car, Plane, Camera, Globe, Coffee, Award, Map, Compass } from 'lucide-react';
...
export default function TourismBookingFlow() {
    ...
    const canProceed = () => {
        ...
        if (step === 2)
            return travelerName && travelerEmail; // Traveler details required, add-ons optional
        if (step === 3)
...
<<<<<<< HEAD
            ...
            <div className="flex-1">
              <h1 className="text-lg font-bold text-gray-900">{tBooking("bookYourExperience")}</h1>
...
<<<<<<< HEAD
        {step === 1 && (<div className="space-y-6">
            {/* Select Package */}
            <div>
              <h2 className="text-xl font-bold text-gray-900 mb-4">{tBooking("chooseYourAdventure")}</h2>
              <div className="space-y-3">
                {packages.map(pkg => (<button key={pkg.id} onClick={() => setSelectedPackage(pkg.id)} className={`w-full bg-white rounded-2xl overflow-hidden border-2 transition-all ${selectedPackage === pkg.id
                    ? 'border-[#083f30] shadow-lg scale-[1.02]'
                    : 'border-gray-200 hover:border-gray-300'}`}>
                    <div className="relative">
                      <img src={pkg.image} alt={pkg.name} className="w-full h-48 object-cover"/>
                      {pkg.popular && (<div className="absolute top-3 left-3 px-3 py-1 bg-gradient-to-r from-[#eacb7f] to-[#d4b76a] text-[#083f30] rounded-lg text-xs font-bold shadow-md flex items-center gap-1">
                          <Star size={12}/>{tBooking("bESTSELLER")}</div>)}
=======
        {step === 1 && (
...
>>>>>>> d8568000f5551fc8b98d4ef0d4dbce5c6f700965
                      ...
                      <div className="mb-3">
<<<<<<< HEAD
                        <div className="text-xs font-semibold text-gray-700 mb-1">{tBooking("includes")}</div>
                        <div className="flex flex-wrap gap-1.5">
                          {pkg.includes.map((item, idx) => (<span key={idx} className="px-2 py-1 bg-gray-50 rounded-md text-xs text-gray-700">
                              • {item}
                            </span>))}
=======
...
<<<<<<< HEAD
            {selectedPackage && (<div>
                ...
                <div className="bg-white rounded-2xl p-4 border border-gray-200">
                  <input type="date" value={selectedDate || ''} onChange={(e) => setSelectedDate(e.target.value)} min={new Date().toISOString().split('T')[0]} className="w-full h-12 px-4 border-2 border-gray-200 rounded-xl focus:border-[#083f30] focus:outline-none font-semibold"/>
                </div>
              </div>)}
            ...
            {/* Number of Travelers */}
            {selectedTime && selectedPackageData?.priceType === 'per person' && (<div>
                <h2 className="text-xl font-bold text-gray-900 mb-4">{tBooking("numberOfTravelers")}</h2>
                <div className="bg-white rounded-2xl p-4 border border-gray-200">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Users size={20} className="text-[#083f30]"/>
                      <span className="font-semibold text-gray-900">{tBooking("travelers")}</span>
                    </div>
                    
                    <div className="flex items-center gap-3">
                      <button onClick={() => setTravelerCount(Math.max(1, travelerCount - 1))} className="w-10 h-10 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center font-bold text-gray-900 transition-colors">
                        -
                      </button>
                      <span className="text-xl font-bold text-gray-900 w-8 text-center">{travelerCount}</span>
                      <button onClick={() => setTravelerCount(travelerCount + 1)} className="w-10 h-10 rounded-full bg-[#083f30] hover:bg-[#0a5a44] flex items-center justify-center font-bold text-white transition-colors">
=======
            {selectedPackage && (
              <div>
...
<<<<<<< HEAD
            ...
            {canProceed() && (<div className="bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-2xl p-4 shadow-md">
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    <h3 className="font-bold text-green-900 mb-2">{tBooking("readyForAdventure")}</h3>
                    <div className="space-y-1.5 text-sm text-green-800">
                      <div className="flex items-center gap-2">
                        <Map size={14} className="flex-shrink-0"/>
                        <span>{tBooking("tour")}{selectedPackageData?.name}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Calendar size={14} className="flex-shrink-0"/>
                        <span>{tBooking("date")}{selectedDate} at {selectedTime}</span>
                      </div>
                      {selectedPackageData?.priceType === 'per person' && (<div className="flex items-center gap-2">
...
>>>>>>> d8568000f5551fc8b98d4ef0d4dbce5c6f700965
                ...
                {/* Continue Button */}
<<<<<<< HEAD
                <button onClick={handleNext} className="w-full h-14 bg-gradient-to-r from-[#083f30] to-[#0a5a44] text-white rounded-xl font-bold hover:shadow-xl active:scale-95 transition-all flex items-center justify-center gap-2 shadow-lg">{tBooking("continueToDetails")}<ChevronRight size={20}/>
                </button>
              </div>)}
          </div>)}
        ...
        {step === 2 && (<div className="space-y-6">
            {/* Traveler Details */}
            <div>
              <h2 className="text-xl font-bold text-gray-900 mb-4">{tBooking("travelerInformation")}</h2>
              <div className="bg-white rounded-2xl p-4 border border-gray-200 space-y-4">
                <div>
                  <label className="text-sm font-semibold text-gray-700 mb-2 block">{tBooking("fullName2")}</label>
                  <input type="text" value={travelerName} onChange={(e) => setTravelerName(e.target.value)} placeholder={tBooking("enterYourFullName")} className="w-full h-12 px-4 border-2 border-gray-200 rounded-xl focus:border-[#083f30] focus:outline-none"/>
                </div>
                
                <div>
                  <label className="text-sm font-semibold text-gray-700 mb-2 block">{tBooking("emailAddress2")}</label>
                  <input type="email" value={travelerEmail} onChange={(e) => setTravelerEmail(e.target.value)} placeholder={tBooking("yourEmailCom")} className="w-full h-12 px-4 border-2 border-gray-200 rounded-xl focus:border-[#083f30] focus:outline-none"/>
=======
                <button
                  onClick={handleNext}
...
<<<<<<< HEAD
              ...
              <p className="text-sm text-gray-600 mb-4">{tBooking("optionalPremiumServicesYouCanSkipThis")}</p>
...
<<<<<<< HEAD
                    ...
                    : 'bg-gray-100 text-gray-900 hover:bg-gray-200'}`}>
                            {selectedAddons.includes(addon.id) ? (<span className="flex items-center justify-center gap-2">
                                ...
                                <Plus size={18}/>{tBooking("addService")}</span>)}
=======
                          <button
...
<<<<<<< HEAD
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-800">{tBooking("addAPrivateGuidePlusPhotographerForATruly")}</p>
                </div>
              </div>
            </div>
          </div>)}
        ...
        {step === 3 && (<div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-lg">
              <div className="p-4 bg-gradient-to-r from-[#083f30] to-[#0a5a44]">
                ...
                <p className="text-sm text-[#eacb7f] mt-1">{tBooking("reviewYourTourDetails")}</p>
=======
                <Compass size={20} className="text-blue-600 flex-shrink-0 mt-0.5" />
                <div>
...
<<<<<<< HEAD
                    ...
                    <h3 className="font-bold text-gray-900">{tBooking("selectedExperience")}</h3>
                  </div>
                  
                  <div className="rounded-xl overflow-hidden border border-gray-200">
                    <img src={selectedPackageData?.image} alt={selectedPackageData?.name} className="w-full h-32 object-cover"/>
=======
                      <Map size={16} className="text-[#083f30]" />
                    </div>
...
<<<<<<< HEAD
                    ...
                    <h3 className="font-bold text-gray-900">{tBooking("tourSchedule")}</h3>
=======
                      <Calendar size={16} className="text-[#083f30]" />
                    </div>
...
<<<<<<< HEAD
                        <Calendar size={16} className="text-[#083f30]"/>
                        <span className="font-semibold">{tBooking("date2")}</span>
=======
                        <Calendar size={16} className="text-[#083f30]" />
...
<<<<<<< HEAD
                        ...
                        <span className="font-semibold">{tBooking("time")}</span>
                      </div>
                      <span className="font-bold text-gray-900">{selectedTime}</span>
                    </div>
                    {selectedPackageData?.priceType === 'per person' && (<div className="flex items-center justify-between p-3 bg-gray-50 rounded-xl">
                        <div className="flex items-center gap-2 text-sm text-gray-700">
                          <Users size={16} className="text-[#083f30]"/>
                          <span className="font-semibold">{tBooking("travelers")}</span>
                        </div>
...
<<<<<<< HEAD
                    ...
                    <h3 className="font-bold text-gray-900">{tBooking("leadTraveler")}</h3>
=======
                      <BadgeCheck size={16} className="text-[#083f30]" />
                    </div>
...
<<<<<<< HEAD
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-800 leading-relaxed">{tBooking("yourPaymentIsProtectedWith256BitSSLEncryption")}</p>
=======
                  <Shield size={20} className="text-green-600 flex-shrink-0 mt-0.5" />
                  <div>
...
<<<<<<< HEAD
                  ...
                  <div>
                    ...
                    <p className="text-sm text-blue-800 leading-relaxed">{tBooking("cancelFreeOfChargeUpTo24HoursBefore")}</p>
=======
                  <BadgeCheck size={20} className="text-blue-600 flex-shrink-0 mt-0.5" />
                  <div>
...
<<<<<<< HEAD
        ...
        {!canProceed() && step === 2 && (<div className="text-center mt-2">
            <span className="text-xs text-gray-500">{tBooking("pleaseEnterYourNameAndEmailToContinue")}</span>
          </div>)}
      </div>
    </div>);
=======
