...
import type { ProviderPageDataResponse, Review } from "@/features/service-providers/types/provider-page-types";
...
/**
 * Interactive provider profile view. The route's server component
 * ({@link ../[id]/page}) fetches the data through the cached repository wrapper
 * and hands it in as `initialData`, so the first paint is real server-rendered
 * HTML — no skeleton → client-fetch → render waterfall. React Query still owns
 * the data afterwards (favourite toggle, review pagination, `refetch()` after a
...
export function ProviderDetailView({ initialData }: { initialData?: ProviderPageDataResponse }) {
  ...
  const { data, error, isFetching, refetch } = useFetchProviderPageData(providerId, locale, currencyOptions, initialData);
