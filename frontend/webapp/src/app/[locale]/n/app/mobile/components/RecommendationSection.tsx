...
export default function RecommendationSection({
  ...
  const RecommendationCard = ({ card }: { card: RecommendationCard }) => (
    ...
    >
      ...
      <div className="min-w-0 flex-1 text-start">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {card.price !== undefined && (
            <div className="whitespace-nowrap text-sm font-bold text-[#083f30] ms-auto">
              {card.currency}
      ...
      <ChevronRight size={16} className="flex-shrink-0 self-center text-gray-400 rtl:rotate-180" />
    </button>
  );
  ...
  return (
    <div className="mb-8 space-y-8">
      {localRecommendations.length > 0 && (
        <div>
          <div className="mb-4">
            <h2 className="mb-1 text-xl font-bold text-gray-900 text-start">{t("localTitle")}</h2>
            <p className="text-sm text-gray-600 text-start">{t("localDescription", { country: displayUserCountry })}</p>
          </div>
      ...
      {internationalRecommendations.length > 0 && (
        <div>
          <div className="mb-4">
            <h2 className="mb-1 text-xl font-bold text-gray-900 text-start">{t("internationalTitle")}</h2>
            <p className="text-sm text-gray-600 text-start">{t("internationalDescription")}</p>
          </div>
