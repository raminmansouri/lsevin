...
export default async function WalletReceiptPage({ params }: WalletReceiptPageProps) {
  ...
  const t = await getTranslations({ locale, namespace: "MobileProfile.receipt" });
      ...
      <div className="max-w-2xl mx-auto px-6 py-10">
        <div className="flex items-start justify-between mb-10 gap-6">
          ...
          <div className="text-right">
            <p className="text-sm text-gray-500 mb-1">{t("reference")}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          <div className="rounded-2xl border border-gray-200 p-5">
            <p className="text-xs uppercase tracking-wide text-gray-500 mb-2">{t("transaction")}</p>
            <p className="font-semibold text-lg">{transaction.title}</p>
            <p className="text-sm text-gray-600 mt-2">{transaction.description}</p>
          </div>

          <div className="rounded-2xl border border-gray-200 p-5">
            <p className="text-xs uppercase tracking-wide text-gray-500 mb-2">{t("provider")}</p>
            <p className="font-semibold text-lg">{transaction.providerName ?? t("lsevinWallet")}</p>
        ...
        <div className="rounded-2xl border border-gray-200 p-5 space-y-3 mb-8">
          ...
          {transaction.bookingReference && (
            <div className="flex items-center justify-between gap-4">
              <span className="text-gray-600">{t("bookingReference")}</span>
