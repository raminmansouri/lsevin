...
const customFetch = async <TResult extends ApiDataValue>(
  ...
  // Server-to-server calls use INTERNAL_API_URL (the direct HTTPS API origin).
  // Using the public URL here caused an HTTP→HTTPS redirect that strips the
  // Authorization header, breaking every authenticated request.
  const finalUrl = new URL(`${serverEnv.INTERNAL_API_URL}/${url}`);
